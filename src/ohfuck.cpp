...
public:
	...
	void imageCallback(const sensor_msgs::ImageConstPtr& depth_image){
		...
		try{
			cv_bridge::toCvCopy(depth, sensor_msgs::image_encodings::MONO16);  
		...
		cv::drayKeypoints( depth_image_cv_ptr->image, depth_image_cv_ptr->image, cv::Scalar(0,0,255), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS );
